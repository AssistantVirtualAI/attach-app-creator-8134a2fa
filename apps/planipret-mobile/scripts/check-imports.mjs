...
const files = [...walk(srcDir), ...walk(sharedDir)];
const missing = [];
let checked = 0;
...
if (missing.length > 0) {
  console.error(red(`\n[check-imports] ✗ ${missing.length} missing local module(s) in apps/planipret-mobile:\n`));
...
console.log(green(`[check-imports] ✓ ${checked} local imports across ${files.length} files all resolve`));

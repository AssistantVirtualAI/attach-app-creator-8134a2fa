/**
 * Planiprêt — Shared Design Tokens (TypeScript)
 * Single source of truth for colors, typography and spacing used across
 * the web app (src/…) and the mobile app (apps/planipret-mobile/…).
 *
 * The CSS custom-property equivalents live in `./tokens.css` and are the
 * runtime source (imported by both `src/index.css` and
 * `apps/planipret-mobile/src/styles.css`).
 *
 * When you need a token in TS/JS (e.g. inline styles, canvas, native
 * bridge, brand config), import from here — do NOT hard-code hex values.
 */

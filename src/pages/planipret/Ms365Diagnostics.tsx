/**
 * Microsoft 365 diagnostics page — fast triage of connection issues.
 * - Shows OK/Limitée/Panne status
 * - Admin config detection (tenant/client/secret)
 * - User token state (email, expiry, scopes)
 * - Callback URL verification
 * - Teams read smoke-test
 * - Reconnect / re-test actions
 */
...
export default function Ms365Diagnostics() {
  ...
  async function testTeams() {
    ...
    try {
      ...
      setTeamsCheck({
        loading: false,
        ok: true,
        message: `${teams.length} équipe(s), ${chats.length} chat(s) lisibles`,
  ...
  const statusColor =
    data?.status === "ok" ? "#2EDC78" : data?.status === "limited" ? "#F5A623" : "#E84C4C";
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-3 mb-3">
          ...
          <Card title="Session utilisateur">
            ...
            <Row
              ok={!!data?.user.connected && !data?.user.expired}
              label="Access token"
              value={
                data?.user.expires_in_sec == null ? "—"
                : data.user.expires_in_sec > 0 ? `Valide (${Math.floor(data.user.expires_in_sec / 60)} min restantes)`
                : `Expiré depuis ${Math.abs(Math.floor(data.user.expires_in_sec / 60))} min`
              }
            />
        ...
        <Card title="URL de callback OAuth">
          <div className="flex items-center gap-2 flex-wrap">
            <code style={{ background: "#040B16", padding: "6px 10px", borderRadius: 6, fontSize: 11, color: "#E8EDF5", border: "1px solid #0E2A45" }}>
              {callbackUrl}
            </code>
            <button
              onClick={() => { navigator.clipboard.writeText(callbackUrl); toast.success("URI copiée"); }}
        ...
        <Card title="Lecture Teams (channels + chats)">
          ...
          {teamsCheck.sample && teamsCheck.sample.length > 0 && (
            <ul className="text-[11px] space-y-1 mt-2" style={{ color: "#8FA8C0" }}>
              {teamsCheck.sample.map((t: any, i: number) => (
                <li key={i}>• {t.team?.displayName ?? t.displayName ?? "?"} — {(t.channels ?? []).length} canaux</li>
              ))}
            </ul>
          )}
        </Card>
        ...
        {/* Last saved test */}
        {data?.last && (
          <Card title="Dernier test enregistré (admin)">
            <Row ok={!!data.last.success} label="Résultat" value={data.last.message ?? "—"} />
            <Row ok={true} label="Testé le" value={data.last.tested_at ? new Date(data.last.tested_at).toLocaleString("fr-CA") : "—"} />
          </Card>
        )}

        {/* Full live panel */}
        <div className="mt-4">
          <Ms365LiveTestPanel onCompleted={refresh} />
        </div>
      </div>
    </div>
  );

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: rows, error } = await admin
      ...
      .select("id, transcript, analyzed_at, ai_summary, ai_coaching, coaching_score, analysis_in_progress, analysis_locked_at, transcript_last_attempt_at, transcript_attempts, ns_call_id, ns_callid, ns_cdr_id, ns_orig_callid, duration_seconds, has_recording, save_consent")
      .or([
      ...
      // Sans consentement approuvé, pp-admin-transcribe refuse l'appel (409
      // call_consent_required) AVANT d'incrémenter transcript_attempts : ces
      // appels seraient re-tentés indéfiniment. On ne les met jamais en file.

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ring_timeout: number = Math.max(30, Math.min(120, Number(body?.ring_timeout ?? 30)));
    ...
    const repairDidRoutes = async (ext: string, domain: string) => {
      ...
      for (const pn of numbers) {
        ...
        if (isVerified) {
          verified += 1;
          details.push({ phone_number: pn, state: "repaired_and_verified", stored_application: afterApp || null, stored_destination: afterDest || null });
        } else {
